...
- [ ] `create()` inserts rate limit record with count=0
- [ ] `get_or_create()` returns existing record or creates new
- [ ] `update_count()` increments applications_count correctly
- [ ] `reset_daily_limits()` sets count to 0 for specified user/platform
- [ ] All operations use `async with session.begin()`
- [ ] Unit tests for create, get, update, reset
- [ ] Integration test: Apply to LinkedIn → verify count incremented
- [ ] Integration test: Apply to LinkedIn hourly limit → verify blocked

**Estimated Time:** 2 hours

**Parallelizable:** YES (with Task 1.7 completed)

---

#### Task 1.9: Implement Failure Logger with Screenshots
**File:** `backend/src/services/failure_logger.py` (NEW FILE)

**What to do:**
- Create `FailureLogger` class with base path configuration
- Implement `log_failure()` method (save screenshot + database log)
- Generate unique filenames (user_id_job_id_timestamp.png)
- Save screenshots to filesystem (logs/auto_apply_failures/)
- Create database log entry with error details
- Implement `cleanup_old_logs()` method (remove logs older than N days)
- Add comprehensive error context (stack trace, platform, job details)

**Acceptance Criteria:**
- [x] FailureLogger class created with base path config
- [x] `log_failure()` saves screenshot to filesystem with unique name
- [x] `log_failure()` creates database entry with error details
- [x] Filenames follow pattern: {user_id}_{job_id}_{timestamp}.png
- [x] Screenshots saved to logs/auto_apply_failures/ directory
- [x] Database log includes: job_id, user_id, platform, error_type, error_message, stack_trace, screenshot_path
- [x] `cleanup_old_logs()` removes logs older than 30 days
- [x] Unit tests for failure logging, screenshot saving, cleanup
- [x] Integration test: Simulate failure → verify screenshot saved
- [ ] `log_failure()` saves screenshot to filesystem with unique name
- [ ] `log_failure()` creates database entry with error details
- [ ] Filenames follow pattern: {user_id}_{job_id}_{timestamp}.png
- [ ] Screenshots saved to logs/auto_apply_failures/ directory
- [ ] Database log includes: job_id, user_id, platform, error_type, error_message, stack_trace, screenshot_path
- [ ] `cleanup_old_logs()` removes logs older than 30 days
- [ ] Unit tests for failure logging, screenshot saving, cleanup
- [ ] Integration test: Simulate failure → verify screenshot saved
- [ ] Integration test: Verify database log entry created with all fields
...
#### Task 1.10: Register AutoApplyService in ServiceRegistry
**File:** `backend/src/services/service_registry.py`

**What to do:**
- Add `AutoApplyServiceProvider` class with per-user service creation
- Implement dependency injection (job_search, job_application, ai, notification, session, rate_limiter)
- Register provider in `_initialize_business_services()` method
- Add `get_auto_apply_service(user_id)` method (returns user-specific instance)
- Implement service initialization (load from DB, setup dependencies)
- Implement service cleanup method (close sessions, cleanup cache)
- Add AutoApplyService to instances dictionary for access

**Acceptance Criteria:**
- [ ] AutoApplyServiceProvider class created with all dependencies
- [ ] Provider registered in `_initialize_business_services()`
- [ ] `get_auto_apply_service(user_id)` method exists and returns service instance
- [ ] Service creates new instance for each user_id call
- [ ] Service initialization loads user config from database
- [ ] Service cleanup closes sessions and clears cache
- [ ] Unit tests for service registration, per-user isolation
- [ ] Integration test: Get service for user1 → verify independent from user2
- [ ] Integration test: Register service → verify accessible via get_auto_apply_service()

**Estimated Time:** 3 hours

**Parallelizable:** NO (depends on Tasks 1.1-1.8)

---

#### Task 1.11: Implement AutoApplyService.run_cycle() Core
**File:** `backend/src/services/auto_apply_service.py`

**What to do:**
- Update constructor to accept all new dependencies (session, rate_limiter, form_filler, failure_logger)
- Implement `run_cycle()` method signature (user_id: str = None)
- Fetch active auto-apply configs from database
- For each active config:
  - Load user session for each platform
  - Search jobs using job_search_service
  - Check duplicate detection (job_id + company + title)
  - Check rate limits per platform
  - Score jobs using AI service (analyze_job_match)
  - Filter by score threshold (default: 70/100)
  - For each matching job:
    - Generate cover letter using AI (generate_cover_letter)
    - Apply using job_application_service
    - Handle success/failure with failure_logger
    - Log activity to database (cycle start, jobs searched, applied, failed)
  - Update rate limit counters
- Send daily summary email using notification service
- Handle all errors with comprehensive logging

**Acceptance Criteria:**
- [ ] `run_cycle()` method implemented with user_id parameter
- [ ] Active configs fetched from database correctly
- [ ] User sessions loaded from SessionManager for each platform
- [ ] Job search calls job_search_service with correct parameters
- [ ] Duplicate detection checks (job_id, company, title) per user
- [ ] Rate limits checked via RateLimiter before each application
- [ ] AI scoring called for each job (analyze_job_match)
- [ ] AI cover letter generation called (generate_cover_letter)
- [ ] Application submitted via job_application_service
- [ ] Success/failure logged via failure_logger
- [ ] Activity log created with cycle statistics
- [ ] Rate limit counters updated after each application
- [ ] Daily summary email sent after cycle completes
- [ ] All operations wrapped in try-catch with logging
- [ ] Unit tests for run_cycle logic (no jobs, rate limit, duplicate)
- [ ] Integration test: Run cycle with 1 job → verify all steps executed
- [ ] Integration test: Run cycle with rate limit → verify blocking
- [ ] Integration test: Run cycle with duplicate → verify skipped

**Estimated Time:** 8 hours

**Parallelizable:** NO (depends on Tasks 1.1-1.11)

---

#### Task 1.12: Fix Email Application with PDF Attachments
**File:** `backend/src/services/job_application_service.py`

**What to do:**
- Update `_apply_via_email()` method to actually send emails
- Extract email address from job description using regex pattern
- Read cover letter content from file
- Get email service from service_registry
- Call `email_service.send_email()` with attachments (resume.pdf, cover_letter.pdf)
- Add error handling for missing emails or send failures
- Log email sending results (success/failure)
- Implement email tracking (store sent email status in database)

**Acceptance Criteria:**
- [ ] Email address extracted from job description correctly
- [ ] Cover letter content read from file
- [ ] Email service retrieved from registry
- [ ] `email_service.send_email()` called with to, subject, body, attachments
- [ ] PDF attachments included: resume.pdf and cover_letter.pdf
- [ ] Error handling for missing emails (returns success: false, error: message)
- [ ] Error handling for send failures (logs exception)
- [ ] Email sending result logged to activity log
- [ ] Unit tests for email extraction, sending, attachments
- [ ] Integration test: Apply via email → verify email received (mock email service)
- [ ] Integration test: Missing email → verify error returned

**Estimated Time:** 2 hours

**Parallelizable:** YES (independent of core run_cycle logic)

---

#### Task 1.13: Integrate Auto-Apply with APScheduler
**File:** `backend/src/services/scheduler_service.py`

**What to do:**
- Add `_register_auto_apply_jobs()` method to SchedulerService
- Fetch all active auto-apply configs from database
- Register per-user interval jobs (every 1 hour)
- Use `IntervalTrigger` for periodic execution
- Use `run_cycle` method as job function
- Pass user_id parameter to each job (enables per-user execution)
- Add logging for job registration (scheduler logs)
- Handle job conflicts (replace_existing=True)
- Implement job pausing/resuming capability

**Acceptance Criteria:**
- [ ] `_register_auto_apply_jobs()` method implemented
- [ ] Active configs fetched from database correctly
- [ ] Per-user jobs registered (one job per active user)
- [ ] IntervalTrigger configured (hours=1 for hourly checks)
- [ ] Each job calls `auto_apply_service.run_cycle(user_id=user_id)`
- [ ] Job registration includes logging (which jobs added)
- [ ] replace_existing=True enabled (update existing jobs)
- [ ] Unit tests for scheduler registration, per-user jobs
- [ ] Integration test: Start scheduler → verify auto-apply jobs registered
- [ ] Integration test: Wait 1 hour → verify run_cycle executed

**Estimated Time:** 3 hours

**Parallelizable:** NO (depends on Tasks 1.1-1.12)

---

#### Task 1.14: Create Enhanced Activity Log Database Model
**File:** `backend/src/database/models.py`

**What to do:**
- Update `DBAutoApplyActivityLog` model (if already exists)
- Add fields: screenshots (Text/JSON array for screenshot paths), errors (Text/JSON array)
- Add fields: cycle_id (unique identifier for cycle), user_id (Foreign Key)
- Define schema with all required fields (id, user_id, cycle_start, cycle_end, jobs_searched, jobs_matched, jobs_applied, applications_successful, applications_failed, errors, screenshots, created_at)
- Add indexes for performance (user_id, cycle_start, created_at)
- Ensure proper Foreign Key relationship to users table
- Add from_dict() and from_model() methods if needed

**Acceptance Criteria:**
- [ ] `DBAutoApplyActivityLog` model defined with all enhanced fields
- [ ] Table name: "auto_apply_activity_logs"
- [ ] Primary key: id (String)
- [ ] Foreign key: user_id (references users.id)
- [ ] Screenshots field: mapped_column(Text) to store JSON array
- [ ] Errors field: mapped_column(Text) to store JSON array
- [ ] cycle_id field: mapped_column(String) for cycle tracking
- [ ] All cycle tracking fields: cycle_start, cycle_end, jobs_searched, jobs_matched, jobs_applied, applications_successful, applications_failed
- [ ] Indexes added: idx_activity_user, idx_activity_cycle, idx_activity_created
- [ ] Migration script created (update table schema if exists)
- [ ] Migration script tested (applies to clean database)
- [ ] Manual verification: Table exists with correct schema
