...
class UserRepository:
        ...
        try:
            ...
            if isinstance(user, dict):
                ...
                if password_hash is None:
                    password_hash = user_data.pop("hashed_password", None)
                user_model = User(
                    ...
                    password_reset_token_expires=user_data.get(
            ...
            db_user = DBUser.from_model(user_model)
            self.session.add(db_user)
            await self.session.commit()
            await self.session.refresh(db_user)

            self.logger.info(f"Created user: {user_model.email}")
    ...
    async def update_password(self, user_id: str, password_hash: str) -> bool:
        ...
        try:
            stmt = (
                ...
                .where(DBUser.id == user_id)
                .values(
                    password_hash=password_hash, updated_at=datetime.now(timezone.utc)
                )
            )
            await self.session.execute(stmt)
            await self.session.commit()
            ...
            await self.session.rollback()
            self.logger.error(
                f"Error updating password for user {user_id}: {e}", exc_info=True
            )
            return False
    ...
    async def update_password_reset_token(
        ...
        try:
            ...
            await self.session.rollback()
            self.logger.error(
                f"Error updating password reset token: {e}", exc_info=True
            )
            return False
    ...
    async def clear_password_reset_token(self, user_id: str) -> bool:
        ...
        try:
            ...
            await self.session.rollback()
            self.logger.error(
                f"Error clearing password reset token: {e}", exc_info=True
            )
            return False
    ...
    async def increment_failed_login_attempts(self, user_id: str) -> int:
        ...
        try:
            ...
            await self.session.rollback()
            self.logger.error(
                f"Error incrementing failed login attempts: {e}", exc_info=True
            )
            return 0
    ...
    async def reset_failed_login_attempts(self, user_id: str) -> bool:
        ...
        try:
            ...
            await self.session.rollback()
            self.logger.error(
                f"Error resetting failed login attempts: {e}", exc_info=True
            )
            return False
