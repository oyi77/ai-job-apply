...
class JobSearchService(JobSearchService):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error initializing job search service: {e}", exc_info=True
            )
            self._initialized = False
        ...
        try:
            ...
            if self._jobspy_available:
                try:
                    ...
                    response = await self._search_with_fallback(
                        request, fallback_reason=str(e)
                    )
            ...
            elapsed_time = time.time() - start_time
            self.logger.error(
                f"Job search failed after {elapsed_time:.2f}s: {e}", exc_info=True
            )
        ...
        for attempt in range(max_retries):
            try:
                ...
                jobs_df = await loop.run_in_executor(
                    None, lambda: scrape_jobs(**jobspy_params)
                )
                ...
                self.logger.warning(f"JobSpy import failed, cannot retry: {e}")
                # Cannot retry import errors
                return await self._search_with_fallback(
                    request, fallback_reason=f"JobSpy not available: {str(e)}"
                )
            except Exception as e:
                error_msg = str(e)
                ...
                if attempt < max_retries - 1 and is_network_error:
                    ...
                    return await self._search_with_fallback(
                        request, fallback_reason=f"JobSpy search failed: {error_msg}"
                    )

        # Should not reach here, but just in case
        return await self._search_with_fallback(
            request, fallback_reason="JobSpy search failed after all retries"
        )

    async def _search_with_fallback(
        self, request: JobSearchRequest, fallback_reason: Optional[str] = None
    ) -> JobSearchResponse:
        """
        ...
        try:
            ...
            jobs_by_platform = {
                k: v for k, v in platform_distribution.items() if v and len(v) > 0
            }
            ...
            self.logger.error(f"Enhanced fallback search failed: {e}", exc_info=True)
    ...
    async def search_specific_site(
        self, site: str, request: JobSearchRequest
    ) -> List[Job]:
        """
    ...
    async def _search_specific_site_with_jobspy(
        self, site: str, request: JobSearchRequest
    ) -> List[Job]:
    ...
    async def _search_specific_site_fallback(
        self, site: str, request: JobSearchRequest
    ) -> List[Job]:
    ...
    def _build_jobspy_params(self, request: JobSearchRequest) -> Dict[str, Any]:
        ...
        if (
            ...
            and request.experience_level in self.experience_mapping
        ):
            ...
            enhanced_keywords = request.keywords + [
                self.experience_mapping[request.experience_level]
            ]
    ...
    def _build_jobspy_detail_params(
        self, platform: str, search_term: str
    ) -> Dict[str, Any]:
        params: Dict[str, Any] = {
            "site_name": [platform],
    ...
    def _convert_jobspy_results(
        self, jobs_df, request: JobSearchRequest
    ) -> JobSearchResponse:
        """Convert JobSpy results to our format."""
        try:
            jobs_by_platform = {}
    ...
    def _convert_job_row(self, row, request: JobSearchRequest) -> Optional[Job]:
        ...
        try:
            ...
            job = Job(
                ...
                application_method=self._determine_application_method(
                    apply_url, contact_email, platform
                ),
                created_at=datetime.now(timezone.utc),
                updated_at=datetime.now(timezone.utc),
            )
    ...
    def _determine_application_method(
        self, apply_url: Optional[str], contact_email: Optional[str], platform: str
    ) -> str:
    ...
    def _generate_mock_jobs(self, request: JobSearchRequest) -> List[Job]:
        ...
        base_titles = []
        if any(
            kw in keyword_str for kw in ["python", "developer", "software", "engineer"]
        ):
            base_titles = [
            ...
            kw in keyword_str for kw in ["data", "analyst", "scientist", "ml", "ai"]
        ):
            base_titles = [
