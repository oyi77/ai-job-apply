...
class JWTAuthService(AuthService):
    ...
    async def initialize(self) -> None:
        ...
        if self._session is None:
            self._session = await database_config.get_session()
    ...
    async def register_user(self, registration: UserRegister) -> TokenResponse:
        ...
        existing_user = await self._repository.get_by_email(registration.email)
        if existing_user:
        ...
        await self._repository.create_session(created_user.id, refresh_token, expires_at)
    ...
    async def login_user(self, login: UserLogin) -> TokenResponse:
        ...
        # Get user by email
        user = await self._repository.get_by_email(login.email)
        if not user:
        ...
        # Create session
        expires_at = datetime.utcnow() + timedelta(days=config.jwt_refresh_token_expire_days)
        await self._repository.create_session(user.id, refresh_token, expires_at)
    ...
    async def refresh_token(self, refresh_token: str) -> TokenResponse:
        ...
        try:
            ...
            session = await self._repository.get_session(refresh_token)
            ...
            await self._repository.invalidate_session(refresh_token)
            ...
            # Create new session
            expires_at = datetime.utcnow() + timedelta(days=config.jwt_refresh_token_expire_days)
            await self._repository.create_session(user.id, new_refresh_token, expires_at)
    ...
    async def update_user_profile(self, user_id: str, updates: UserProfileUpdate) -> UserProfile:
        ...
        if updates.email:
            existing_user = await self._repository.get_by_email(updates.email)
    ...
    async def change_password(self, user_id: str, password_change: PasswordChange) -> bool:
        ...
        # Update password
        success = await self._repository.update_password(user_id, new_password_hash)
        
        if success:
            # Invalidate all sessions (force re-login)
            await self._repository.invalidate_all_user_sessions(user_id)
    ...
    async def request_password_reset(self, reset_request: PasswordResetRequest) -> bool:
        ...
        # Get user by email
        user = await self._repository.get_by_email(reset_request.email.lower())
        ...
        await self._repository.update_password_reset_token(user.id, reset_token, expires_at)
    ...
    async def reset_password(self, reset: PasswordReset) -> bool:
        ...
        async with database_config.get_session() as session:
            ...
            if not db_user.password_reset_token_expires or db_user.password_reset_token_expires < datetime.utcnow():
        ...
        success = await self._repository.update_password(user_id, new_password_hash)
        if success:
            await self._repository.clear_password_reset_token(user_id)
            # Invalidate all sessions (force re-login)
            await self._repository.invalidate_all_user_sessions(user_id)
