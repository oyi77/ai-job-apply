...
class JWTAuthService(AuthService):
    ...
    async def register_user(self, registration: UserRegister) -> TokenResponse:
        """Register a new user."""
        self.logger.info(f"=== REGISTER USER CALLED ===")
        self.logger.info(f"Email: {registration.email}")
        self.logger.info(f"Password: {registration.password}")
        self.logger.info(f"Name: {registration.name}")
        ...
        async with self._get_session_repo() as (session, repository):
            ...
            if existing_user:
                self.logger.warning(f"User already exists: {existing_user.email}")
            ...
            self.logger.debug(
                f"Tokens created - access: {access_token[:20]}..., refresh: {refresh_token[:20]}..."
            )
            ...
            self.logger.debug(f"Session created - expires at: {expires_at}")
            ...
            self.logger.debug(f"Returning token response: {response.model_dump()}")
            return response
        ...
        async with self._get_session_repo() as (session, repository):
            ...
            if not password_valid:
                ...
                if config.account_lockout_enabled:
                    failed_attempts = await repository.increment_failed_login_attempts(
                        user.id
                    )
                    if failed_attempts >= config.max_failed_login_attempts:
                        ...
                        self.logger.warning(
                            f"Account locked due to {failed_attempts} failed login attempts: {user.email}"
                        )
                        raise ValueError(
                            f"Too many failed login attempts. Account locked for {config.account_lockout_duration_minutes} minutes."
                        )
                raise ValueError("Invalid email or password")
    ...
    async def logout_user(
        self, refresh_token: str, user_id: Optional[str] = None
    ) -> bool:
    ...
    async def request_password_reset(self, reset_request: PasswordResetRequest) -> bool:
        ...
        async with self._get_session_repo() as (session, repository):
            ...
            await repository.update_password_reset_token(
            ...
            if self.email_service:
                ...
                self.logger.warning(
                    "Email service not available, strictly logging token (dev mode only)"
                )
    ...
    async def reset_password(self, reset: PasswordReset) -> bool:
        ...
        async with self._get_session_repo() as (session, repository):
            ...
            if (
                not db_user.password_reset_token
                or db_user.password_reset_token != reset.token
            ):
                raise ValueError("Invalid reset token")
