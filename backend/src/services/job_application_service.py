...
from src.services.platform_handlers import (
...
class MultiPlatformJobApplicationService(JobApplicationService):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if await self.browser_service.initialize():
                ...
                self.logger.info("Browser automation service initialized successfully")
            else:
                self.logger.warning("Browser automation service failed to initialize")
            self.logger.info("Job application service initialized successfully")
        except Exception as e:
            self.logger.error(
                f"Error initializing job application service: {e}", exc_info=True
            )
            self._initialized = False
    ...
    async def get_application_info(self, job: Job) -> ApplicationInfo:
        ...
        try:
            self.logger.info(
                f"Extracting application info for job: {job.title} at {job.company}"
            )
            ...
            return ApplicationInfo(
                job=job, application_method=JobApplicationMethod.UNKNOWN
            )
    ...
    async def apply_to_job(
        ...
        try:
            ...
            handler = self.platform_handlers.get(
                platform, self._handle_generic_application
            )
            ...
            application_id = (
                f"{platform}_{job.id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
            )
    ...
    async def validate_application_data(
        self, job: Job, resume: Resume, cover_letter: str
    ) -> Dict[str, Any]:
        """
        ...
        if job.requirements and resume.skills:
            missing_skills = [
                ...
                if req.lower() not in [skill.lower() for skill in resume.skills]
            ]
            if missing_skills:
    ...
    async def _handle_linkedin_application(
        self,
        job: Job,
        resume: Resume,
        cover_letter: str,
        ...
        """Handle LinkedIn job applications with real browser automation."""
        try:
            self.logger.info(f"Applying to LinkedIn job: {job.title} at {job.company}")

            # Check if browser automation is available
            if not self._browser_initialized:
                self.logger.warning(
                    "Browser automation not initialized, using fallback"
                )
                return await self._apply_via_external_url(
                    job, resume, cover_letter, additional_data
                )

            # Create LinkedIn handler
            handler = LinkedInHandler(
                self.browser_service.browser_manager,
                self.browser_service.field_detector,
            )

            # Prepare profile data from resume
            profile_data = self._prepare_profile_data(resume, cover_letter)

            # Apply using browser automation
            result = await handler.handle_application(
                job_url=str(job.url),
                resume_path=resume.file_path or "",
                cover_letter_path="",
    ...
    async def _handle_indeed_application(
        self,
        job: Job,
        resume: Resume,
        cover_letter: str,
        ...
        """Handle Indeed job applications with real browser automation."""
        try:
            self.logger.info(f"Applying to Indeed job: {job.title} at {job.company}")
    ...
    async def _handle_glassdoor_application(
        self,
        job: Job,
        resume: Resume,
        cover_letter: str,
        ...
        """Handle Glassdoor job applications with real browser automation."""
        try:
            self.logger.info(f"Applying to Glassdoor job: {job.title} at {job.company}")
            ...
            # Create Glassdoor handler
            handler = GlassdoorHandler(
                self.browser_service.browser_manager,
                self.browser_service.field_detector,
            )
    ...
    async def _handle_google_jobs_application(
        self,
        job: Job,
        resume: Resume,
        cover_letter: str,
        ...
        try:
            ...
            if job.apply_url:
                ...
                return {
                    "success": False,
                    "error": "No application URL available for Google Jobs",
                }
        except Exception as e:
            self.logger.error(f"Google Jobs application failed: {e}")
            return {
                "success": False,
                "error": f"Google Jobs application failed: {str(e)}",
            }

    async def _handle_ziprecruiter_application(
        self,
        job: Job,
        resume: Resume,
        cover_letter: str,
        ...
        try:
            ...
            if job.external_application and job.apply_url:
                ...
                return await self._apply_via_ziprecruiter_form(
                    job, resume, cover_letter, additional_data
                )
        except Exception as e:
            ...
            return {
                "success": False,
                "error": f"ZipRecruiter application failed: {str(e)}",
            }

    async def _handle_mock_application(
        self,
        job: Job,
        resume: Resume,
        cover_letter: str,
    ...
    async def _handle_generic_application(
        self,
        job: Job,
        resume: Resume,
        cover_letter: str,
        ...
        try:
            if job.apply_url:
                ...
                return await self._apply_via_email(
                    job, resume, cover_letter, additional_data
                )
            else:
    ...
    async def _apply_via_external_url(
        self,
        job: Job,
        resume: Resume,
        cover_letter: str,
    ...
    async def _determine_application_method(
        self, job: Job, application_form: Optional[ApplicationForm]
    ...
    def _prepare_profile_data(
        self, resume: Resume, cover_letter: str
    ) -> Dict[str, Any]:
        """Prepare profile data from resume for form filling."""
        profile_data = {}

        # Extract name from resume content if available
        if resume.content:
            # Try to extract name from first line or similar
            lines = resume.content.strip().split("\n")
            if lines:
                profile_data["full_name"] = lines[0].strip()

        # Add skills if available
        if resume.skills:
            profile_data["skills"] = resume.skills

        # Add experience if available
        if hasattr(resume, "experience") and resume.experience:
            profile_data["experience"] = str(resume.experience)

        # Add education if available
        if hasattr(resume, "education") and resume.education:
            profile_data["education"] = resume.education

        # Add contact info if available
        if hasattr(resume, "email") and resume.email:
            profile_data["email"] = resume.email

        if hasattr(resume, "phone") and resume.phone:
            profile_data["phone"] = resume.phone
