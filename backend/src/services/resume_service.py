...
class ResumeService(ResumeService):
    ...
    async def upload_resume(
        self, file_path: str, name: str, user_id: Optional[str] = None
    ) -> Resume:
        ...
        try:
            ...
            if content:
                ...
                try:
                    ...
                    self.logger.warning(
                        f"AI extraction not available, using regex: {e}"
                    )
                    education = self._extract_education(content)
                    certifications = self._extract_certifications(content)
            ...
            self.logger.info(
                f"Resume uploaded successfully: {resume.name} (ID: {resume.id})"
            )
            return resume
    ...
    async def get_resume(
        ...
        try:
            ...
            if cached_resume is not NO_VALUE:
                self.logger.debug(f"Cache hit for resume: {resume_id}")
                if cached_resume and user_id and cached_resume.user_id != user_id:
                    self.logger.warning(
                        f"Cached resume {resume_id} does not match user {user_id}"
                    )
                else:
    ...
    async def get_all_resumes(self, user_id: Optional[str] = None) -> List[Resume]:
        ...
        try:
            if user_id:
                ...
                if cached_resumes is not NO_VALUE:
    ...
    async def set_default_resume(
        ...
        try:
            ...
            if not resume:
                self.logger.warning(
                    f"Cannot set default, resume not found: {resume_id}"
                )
                return False

            if self.repository:
                return await self.repository.set_default_resume(
                    resume_id, user_id=user_id
                )

            # Fallback to in-memory
            previous_default_id = self.default_resume_id
            if previous_default_id:
                current_default = self.resumes.get(previous_default_id)
                if current_default:
            ...
            if previous_default_id:
                cache_region.delete(f"resume:{previous_default_id}")
            if user_id:
                cache_region.delete(f"user_resumes:{user_id}")
            ...
            self.logger.error(
                f"Error setting default resume {resume_id}: {e}", exc_info=True
            )
            return False
    ...
    async def extract_text(self, resume: Resume) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"Error extracting text from resume {resume.id}: {e}", exc_info=True
            )
            return ""
    ...
    async def extract_text_from_file(self, file_path: str) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"Error extracting text from file {file_path}: {e}", exc_info=True
            )
            return ""

    async def update_resume(
        self, resume_id: str, updates: dict, user_id: Optional[str] = None
    ) -> Optional[Resume]:
            ...
            if self.repository:
                ...
                if update_dict:
                    resume = await self.repository.update(
                        resume_id, update_dict, user_id=user_id
                    )
    ...
    async def bulk_delete_resumes(
        self, resume_ids: List[str], user_id: Optional[str] = None
    ) -> bool:
    ...
    async def _extract_text_from_pdf(self, file_path: str) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"Error extracting text from PDF {file_path}: {e}", exc_info=True
            )
            return ""
    ...
    async def _extract_text_from_docx(self, file_path: str) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"Error extracting text from DOCX {file_path}: {e}", exc_info=True
            )
            return ""
    ...
    async def _extract_text_from_txt(self, file_path: str) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"Error extracting text from TXT {file_path}: {e}", exc_info=True
            )
            return ""
    ...
    def _extract_education(self, content: str) -> List[str]:
        ...
        try:
            ...
            for pattern in degree_patterns:
                ...
                education.extend(
                    [m.title() if isinstance(m, str) else m for m in matches]
                )
            ...
            for pattern in university_patterns:
                ...
                filtered = [
                    ...
                    for m in matches
                    if len(m.strip()) > 5
                    and "university" in m.lower()
                    or "college" in m.lower()
                ]
        ...
        try:
            ...
            for pattern in cert_patterns:
                ...
                certifications.extend(
                    [m.title().strip() if isinstance(m, str) else m for m in matches]
                )
