...
class Resume(BaseModel):
    ...
    skills: Optional[List[str]] = Field(
        None, description="Skills extracted from resume"
    )
    ...
    is_default: bool = Field(
        default=False, description="Whether this is the default resume"
    )
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)
    user_id: Optional[str] = Field(
        None, description="ID of the user who owns this resume"
    )
...
class ResumeOptimizationRequest(BaseModel):
    ...
    resume_content: Optional[str] = Field(
        None, description="Current resume text content"
    )
    optimization_focus: Optional[List[str]] = Field(
        None, description="Areas to focus optimization on"
    )
...
class ResumeOptimizationResponse(BaseModel):
    ...
    confidence_score: float = Field(
        ..., ge=0.0, le=1.0, description="Optimization confidence score"
    )
    ats_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="ATS compatibility score (0-1)"
    )
    ats_checks: Optional[Dict[str, float]] = Field(
        None, description="Individual ATS check scores"
    )
    ats_recommendations: Optional[List[str]] = Field(
        None, description="ATS-specific recommendations"
    )
