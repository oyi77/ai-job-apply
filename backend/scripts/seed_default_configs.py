        ...
        for config_data in DEFAULT_CONFIGS:
            try:
                ...
                if existing is None:
                    ...
                    seeded += 1
                    print(f"  ✅ Created: {config_data['name']} ({config_data['key']})")
                else:
                    skipped += 1
                    print(f"  ⏭️  Skipped: {config_data['name']} (already exists)")
            except Exception as e:
                print(f"  ❌ Error creating {config_data['key']}: {e}")
        
        print(f"\n✅ Seeding complete! Created: {seeded}, Skipped: {skipped}")
        print("\n💡 Tip: Update API keys via the API or frontend:")
