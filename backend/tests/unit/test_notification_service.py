...
from src.services.scheduler_service import ReminderType
    ...
    async def test_send_email_with_attachments(self, email_service):
        ...
        with patch.object(email_service, '_send_smtp', new_callable=AsyncMock) as mock_send:
            ...
            result = await email_service.send_email(
                ...
                html_content='<p>Test content</p>',
                attachments=[
                    {'filename': 'resume.pdf', 'content': b'PDF content', 'mime_type': 'application/pdf'}
                ]
            )
...
class TestNotificationService:
    ...
    @pytest.mark.asyncio
    async def test_send_reminder_success(self, notification_service, mock_email_service):
        """Test sending reminder notification successfully."""
        with patch.object(notification_service.template_renderer, 'render_reminder_email', return_value={
    ...
    @pytest.mark.asyncio
    async def test_send_reminder_failure(self, notification_service, mock_email_service):
    ...
    @pytest.mark.asyncio
    async def test_send_interview_prep_notification(self, notification_service, mock_email_service):
    ...
    @pytest.mark.asyncio
    async def test_send_status_check_notification(self, notification_service, mock_email_service):
    ...
    @pytest.mark.asyncio
    async def test_send_follow_up_reminder(self, notification_service, mock_email_service):
