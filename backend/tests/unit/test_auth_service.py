...
async def auth_service():
    ...
    service._get_session_repo.return_value.__aenter__.return_value = (
        mock_session,
        mock_repo,
    )
...
class TestUserRegistration:
    ...
    async def test_register_user_duplicate_email(self, auth_service, sample_user):
        ...
        registration = UserRegister(
            email="existing@example.com", password="Password123", name="New User"
        )
    ...
    async def test_change_password_success(self, auth_service, sample_user):
        ...
        with patch.object(auth_service, "_verify_password", return_value=True):
            success = await auth_service.change_password(
                sample_user.id, password_change
            )
...
class TestLogout:
    ...
    async def test_logout_success(self, auth_service, sample_user):
        ...
        assert success is True
        auth_service._repository.invalidate_session.assert_called_once_with(
            refresh_token
        )
...
class TestDeleteUser:
    ...
    async def test_delete_user_success(self, auth_service, sample_user):
        ...
        success = await auth_service.delete_user(sample_user.id, "test_password")
    ...
    async def test_delete_user_not_found(self, auth_service):
        ...
        with pytest.raises(ValueError, match="User not found"):
            await auth_service.delete_user("nonexistent-id", "password")

    @pytest.mark.asyncio
    async def test_logout_wrong_user(self, auth_service, sample_user):
        """Test logout with session belonging to different user - returns False for security."""
        refresh_token = "test-refresh-token"
        ...
        # Logout returns False when refresh token belongs to different user (security)
        assert success is False
